...
/**
 * 同步SignPicture数据包（服务端 → 客户端）
 *
 * 流程：
 * 1. 服务端创建/更新SignPicture后广播此包
 * 2. 客户端接收并保存元数据到本地缓存
 * 3. 客户端触发图片下载（使用ContentManager）
 */
public class SyncSignPicturePacket {
    ...
    public SyncSignPicturePacket(SignPictureData data) {
    ...
    public static void encode(SyncSignPicturePacket packet, FriendlyByteBuf buf) {
        ...
        buf.writeLong(packet.createdTime);
        buf.writeLong(packet.lastModified);
    }

    public static SyncSignPicturePacket decode(FriendlyByteBuf buf) {
        ...
        return new SyncSignPicturePacket(data);
    }
    ...
    private static void handleClient(SyncSignPicturePacket packet) {
        try {
            // 1. 重建数据对象
